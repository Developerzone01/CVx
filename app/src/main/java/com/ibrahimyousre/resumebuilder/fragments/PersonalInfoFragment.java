...
public class PersonalInfoFragment extends ResumeFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        EditText personalstatement = root.findViewById(R.id.ps1);
        personalstatement.setText(personalInfo.getpersonalstatement());
        personalstatement.addTextChangedListener(new TextChangeListener() {
            @Override
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                personalInfo.setPersonalstatement(s.toString());
            }
        });

        EditText dateofbirth = root.findViewById(R.id.dob);
        dateofbirth.setText(personalInfo.getDateofbirth());
        dateofbirth.addTextChangedListener(new TextChangeListener() {
            @Override
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                personalInfo.setDateofbirth(s.toString());
            }
        });

        EditText currentresidence = root.findViewById(R.id.cr);
        currentresidence.setText(personalInfo.getCurrentresidence());
        currentresidence.addTextChangedListener(new TextChangeListener() {
            @Override
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                personalInfo.setCurrentresidence(s.toString());
            }
        });
        return root;
    }
